...
def _build_gemini_prompt(
    ...
    return (
        "Write one concise operational weather discussion (roughly 90-140 words) "
        "for the supplied route. Lead with the route duration and distance, then "
        "summarize the overall weather ranges and only the important trends or "
        "changes listed in ROUTE_FINDINGS. Do NOT enumerate every waypoint or repeat "
        "each waypoint's latitude, longitude, ETA, and measurements. Use ROUTE_OVERVIEW "
        "as the primary source; use ROUTE_DATA only when needed to explain a material "
        "trend. Use only supplied facts and numeric values. Do not estimate or invent "
        "fronts, sea state, clouds, gusts, locations, or any other conditions not in "
        "the data. Mention missing values as unavailable. Return only the discussion, "
        "with no heading, bullet list, disclaimer, markdown, or source citations.\n\n"
