...
async function assertWeatherInputsAreNumericWhenPresent(rows, rowCount) {
  ...
  expect(
    populatedFieldCount,
    "Expected at least one populated weather value across the rendered waypoint table."
  ).toBeGreaterThan(0);
}
...
test("Small forecast test with data validation", async ({ page }) => {
  ...
  await assertWeatherInputsAreNumericWhenPresent(rows, smallWaypoints.length);
...
test("Prefilled waypoints test", async ({ page }) => {
  ...
  await assertWeatherInputsAreNumericWhenPresent(rows, prefilledWaypoints.length);
...
// 50 waypoints (confirm rows render and populated values remain numeric)
test("Large forecast test", async ({ page }, testInfo) => {
  ...
  await assertWeatherInputsAreNumericWhenPresent(rows, largeWaypoints.length);
